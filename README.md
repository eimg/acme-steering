...
The root launcher supplies Prelude's default URL automatically. Prelude's **Connections** screen can inspect, test, override, disable, or return to that startup setting. Its credential probe exercises the same product-specific notification permission without creating a workflow event; tokens remain server-side.
