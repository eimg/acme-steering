...
Steering sends every source-backed resolution to `POST /api/steering/decisions`, including approve, reject, request revision, defer, escalate, and cancel. Human-required cases carry the human actor and rationale; an explicitly policy-authorized automatic approval carries the attributable Steering service principal and policy rationale. The notice contains a stable decision id, case and action correlation, resolution, rationale, actor identity, decision time, resource identity, and the source revision that was evaluated. It contains no generic state mutation or instruction for what the source must do next.
