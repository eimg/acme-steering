...
Decision delivery is idempotent, but the first pass is synchronous and has no durable delivery queue. An unavailable receipt remains visible in the Steering case rather than being presented as delivered. Automated retry and source-specific handling policies are later increments; a product's manual workflow remains available throughout.
