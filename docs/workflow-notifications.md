...
Steering appends every accepted event to its Activity journal. An `open` case event creates or refreshes a case at the newest source revision. An unchanged notification does not erase a human rejection, deferment, revision request, or escalation; a newer source revision reopens the case with prior decision-delivery metadata cleared. `resolved`, `withdrawn`, and `superseded` events close the corresponding case and append a system-authored remark. Duplicate event ids are acknowledged without repeating either effect.
