...
A source configuration screen may validate its delivery credential without creating an event by calling `POST /api/notifications/check` with its product name. The endpoint applies the same product-specific notification permission used for ingestion and returns no secret material.
